...
[English](../en/quick-start.md) · [首页](../../README.zh-CN.md)
...
需要在 PC 上自行生成并逐项验收两套 Windows 安装包时，按 [Windows 本地打包与验收](windows-local-build.md) 执行。两个 Edition 使用同一个应用标识，不能作为两个独立应用并排安装。
...
点击右侧 **AI → 设置**，选择服务商和模型、输入 API Key，然后保存。保存不会发起模型请求，发送消息才会。外部 MCP 客户端是同一设置页中的另一种接入方式；分享配对配置前先读 [AI 与 MCP](ai-guide.md)。
...
反馈时提供应用版本或源码提交、系统、数据源、复现步骤和可见错误。截图或日志中移除 API Key、MCP token 和个人路径。常见能力边界见[常见问题](faq.md)。
