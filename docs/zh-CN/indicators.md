...
[English](../en/indicators.md) · [首页](../../README.zh-CN.md) · [AI 工具](api-reference.md)
...
点数上限不等于 CPU 许可：不要每个 tick 都重写数千个未变化点，优先 `series.update` 与实际受影响区间的有界重算，不要反复重启故障指标。当前权威数值见 [limits.ts](../../src/user-indicator-runtime/limits.ts)，这些是工程保护，不是订阅配额。

更多完整例子：[副图](../../fixtures/user-indicators/02-range-pane.tfi)、[标记](../../fixtures/user-indicators/03-marker-style.tfi)、[Canvas](../../fixtures/user-indicators/04-canvas.tfi)、[Panel](../../fixtures/user-indicators/05-panel.tfi)。超出运行时的需求，通过另行授权的[源码扩展](extensions.md)实现，不编造 `.tfi` API。
