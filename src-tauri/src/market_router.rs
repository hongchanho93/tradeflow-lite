...
use crate::market_adapter::HistoryResponse;
#[cfg(any(tradeflow_tdx, test))]
use crate::market_adapter::QuoteAdapter;
#[cfg(tradeflow_tdx)]
use crate::market_adapter::TDX_PROVIDER_DESCRIPTOR;
#[cfg(tradeflow_tdx)]
...
impl MarketEdition {
    pub fn from_name(value: &str) -> Self {
        match value {
            "global" => Self::Global,
            "cn" => Self::Cn,
            _ if cfg!(tradeflow_tdx) => Self::Cn,
            _ => Self::Global,
        }
    }

    pub fn compiled() -> Self {
        Self::from_name(option_env!("TRADEFLOW_LITE_EDITION").unwrap_or(if cfg!(tradeflow_tdx) {
            "cn"
        } else {
            "global"
        }))
    ...
    fn default_provider_ids(self) -> &'static [&'static str] {
        match self {
            Self::Cn => &[
                "tdx",
                "binance_spot",
...
pub struct MarketEditionInfo {
    pub edition: MarketEdition,
    pub default_enabled_provider_ids: Vec<String>,
}
...
pub struct MarketRouter {
    ...
    enabled_provider_ids: Arc<RwLock<HashSet<String>>>,
}
...
impl MarketRouter {
    ...
    pub fn builtin() -> Result<Self, AppError> {
        let registry = AdapterRegistry::new(REGISTERED_ADAPTERS.iter().copied())?;
        let edition = MarketEdition::compiled();
        let enabled_provider_ids = edition
            .default_provider_ids()
            .iter()
            .filter_map(|id| {
                registry
                    .descriptors()
                    .find(|descriptor| descriptor.id == *id && descriptor.enabled)
                    .map(|_| (*id).to_string())
            })
            .collect();
        Ok(Self {
            registry,
            edition,
            enabled_provider_ids: Arc::new(RwLock::new(enabled_provider_ids)),
        })
    }
    ...
    pub fn new(
        ...
        let registry = AdapterRegistry::new(registrations)?;
    ...
    pub fn provider_descriptors(&self) -> Vec<ProviderDescriptor> {
        self.registry
            ...
            .map(|descriptor| ProviderDescriptor {
                enabled: descriptor.enabled && self.is_provider_enabled(descriptor.id),
                ..*descriptor
            })
            .collect()
    }

    pub fn edition_info(&self) -> MarketEditionInfo {
        MarketEditionInfo {
            edition: self.edition,
            default_enabled_provider_ids: self
                .edition
                .default_provider_ids()
                .iter()
                .map(|id| (*id).to_string())
    ...
    pub fn require_provider_enabled(&self, provider_id: &str) -> Result<(), AppError> {
        let descriptor = self
            .registry
            .descriptors()
            ...
            .ok_or_else(|| {
                AppError::new(
                    "provider_not_found",
                    format!("provider {provider_id} 未注册"),
                )
            })?;
        if !descriptor.enabled {
            return Err(AppError::new(
                "market_data_source_unavailable",
                format!("{} 行情适配器未编译进当前构建", descriptor.display_name),
            ));
        }
        if !self.is_provider_enabled(provider_id) {
            return Err(AppError::new(
                "market_data_source_disabled",
                format!("{} 行情源当前未启用", descriptor.display_name),
            ));
        }
        Ok(())
    }
    ...
    pub fn set_provider_enabled(
        ...
        Ok(ProviderDescriptor {
            enabled: descriptor.enabled && enabled,
            ..descriptor
        })
    }

    pub fn fetch_history(&self, request: HistoryRequest) -> Result<HistoryResponse, AppError> {
        self.require_provider_enabled(&request.provider_id)?;
...
impl Clone for MarketRouter {
    fn clone(&self) -> Self {
        Self {
            ...
            edition: self.edition,
            enabled_provider_ids: self.enabled_provider_ids.clone(),
        }
    }
}
...
mod tests {
    ...
    use super::{
        ...
        HistoryResponse, MarketDataAdapter, MarketEdition, MarketRouter, ProviderDescriptor, QuoteAdapter,
    ...
    fn provider_descriptors_declare_real_facets() {
        ...
        let expected = 4
            + usize::from(cfg!(tradeflow_tdx))
            + usize::from(cfg!(feature = "provider-polymarket"));
        ...
        let tdx = descriptors.iter().find(|item| item.id == "tdx");
        assert_eq!(tdx.is_some(), cfg!(tradeflow_tdx));
        if let Some(tdx) = tdx {
    ...
    #[test]
    fn editions_define_provider_defaults_and_compile_provider_boundary() {
        let cn = MarketEdition::from_name("cn");
        let global = MarketEdition::from_name("global");
        assert!(cn.default_provider_ids().contains(&"tdx"));
        assert!(!global.default_provider_ids().contains(&"tdx"));
        assert!(global.default_provider_ids().contains(&"binance_spot"));

        let router = MarketRouter::builtin().unwrap();
        let ids: Vec<_> = router.registry().descriptors().map(|descriptor| descriptor.id).collect();
        assert_eq!(ids.contains(&"tdx"), cfg!(tradeflow_tdx));
        assert!(ids.contains(&"binance_spot"));
        assert!(ids.contains(&"okx_spot"));
    }

    #[test]
    fn compiled_edition_applies_runtime_defaults() {
        let router = MarketRouter::builtin().unwrap();
        let descriptors = router.provider_descriptors();
        let enabled = |id: &str| descriptors.iter().find(|item| item.id == id).is_some_and(|item| item.enabled);
        match MarketEdition::compiled() {
            MarketEdition::Cn => assert!(enabled("tdx")),
            MarketEdition::Global => {
                assert!(!enabled("tdx"));
                if cfg!(feature = "provider-binance") { assert!(enabled("binance_spot")); }
            }
            MarketEdition::Custom => unreachable!("compiled edition is never custom"),
        }
    }

    #[test]
    fn runtime_provider_policy_blocks_requests_and_can_reenable_adapter() {
        let router = MarketRouter::new([AdapterRegistration::new(&FAKE_ADAPTER)]).unwrap();
        assert!(router.is_provider_enabled("fake"));
        router.set_provider_enabled("fake", false).unwrap();
        assert!(!router.is_provider_enabled("fake"));
        let error = router.fetch_history(HistoryRequest {
        ...
        assert_eq!(error.code, "market_data_source_disabled");

        router.set_provider_enabled("fake", true).unwrap();
        assert!(router.is_provider_enabled("fake"));
        assert!(router.provider_descriptors().iter().any(|descriptor| descriptor.id == "fake" && descriptor.enabled));
    }
