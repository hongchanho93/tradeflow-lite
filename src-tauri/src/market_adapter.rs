...
/// Returns the chart bar opening time (Unix seconds) for a trade timestamp.
/// Crypto providers use UTC candle boundaries; weekly bars start Monday 00:00 UTC.
pub fn trade_bar_open_time(trade_time_ms: i64, resolution: Resolution) -> Option<i64> {
    if resolution == Resolution::Month {
        let trade = chrono::DateTime::<Utc>::from_timestamp_millis(trade_time_ms)?;
        return Utc
            .with_ymd_and_hms(trade.year(), trade.month(), 1, 0, 0, 0)
            .single()
...
/// Returns the chart bar timestamp used by provider OHLCV rows for a trade.
/// Crypto chart bars are keyed by their exclusive UTC close boundary.
pub fn trade_bar_time(trade_time_ms: i64, resolution: Resolution) -> Option<i64> {
    if resolution == Resolution::Month {
        let trade = chrono::DateTime::<Utc>::from_timestamp_millis(trade_time_ms)?;
        let (year, month) = if trade.month() == 12 {
            (trade.year().checked_add(1)?, 1)
        } else {
    ...
    let open_time = trade_bar_open_time(trade_time_ms, resolution)?;
    ...
    open_time.checked_add(interval_seconds)
}
...
pub trait CatalogAdapter: Sync {
    ...
    fn list_symbols_page(&self, request: CatalogPageRequest) -> Result<CatalogPage, AppError> {
        if request.limit == 0 || request.limit > 1_000 {
            return Err(AppError::new("catalog_page_invalid", "目录分页大小超出安全范围"));
        }
        let offset = match request.cursor.as_deref() {
            None => 0,
            Some(value) if !value.is_empty() && value.bytes().all(|byte| byte.is_ascii_digit()) => value
                .parse::<usize>()
                .map_err(|_| AppError::new("catalog_cursor_invalid", "目录分页游标无效"))?,
            Some(_) => return Err(AppError::new("catalog_cursor_invalid", "目录分页游标无效")),
        };
        let symbols = self.list_symbols(CatalogRequest {
            provider_id: request.provider_id,
            venue: request.venue,
        })?;
        if offset >= symbols.len() && offset != 0 {
            return Err(AppError::new("catalog_cursor_invalid", "目录分页游标已超出结果范围"));
        }
        let end = offset.saturating_add(request.limit).min(symbols.len());
        Ok(CatalogPage {
            symbols: symbols[offset..end].to_vec(),
            next_cursor: (end < symbols.len()).then(|| end.to_string()),
        })
    }
}
...
mod tests {
    use super::{
        ADAPTER_CONTRACT_VERSION, AdapterRegistration, AdapterRegistry, CatalogAdapter,
        CatalogPageRequest, CatalogRequest, CatalogSymbol, HistoryRequest, HistoryResponse,
        MarketDataAdapter, ProviderCapabilities, ProviderDescriptor, is_valid_provider_id,
        trade_bar_open_time, trade_bar_time,
    };
    ...
    struct PagedCatalogAdapter;

    impl CatalogAdapter for PagedCatalogAdapter {
        fn list_symbols(&self, request: CatalogRequest) -> Result<Vec<CatalogSymbol>, AppError> {
            Ok((0..205)
                .map(|index| CatalogSymbol {
                    provider_id: request.provider_id.clone(),
                    symbol: format!("{}:S{index:03}", request.venue),
    ...
    #[test]
    fn default_catalog_adapter_pages_large_catalogs() {
        let adapter = PagedCatalogAdapter;
        let first = adapter
            ...
            .unwrap();
        assert_eq!(first.symbols.len(), 100);
        ...
        let second = adapter
            ...
            .unwrap();
        assert_eq!(second.symbols.len(), 100);
        ...
        let last = adapter
            ...
            .unwrap();
        assert_eq!(last.symbols.len(), 5);
        assert_eq!(last.next_cursor, None);
    }

    #[test]
    fn trade_bar_time_is_the_opening_time_for_intraday_week_and_month() {
        assert_eq!(
            trade_bar_open_time(1_789_375_265_432, Resolution::Minute1),
            Some(1_789_375_260)
        );
        let monday = chrono::DateTime::parse_from_rfc3339("2026-09-14T00:00:00Z")
            .unwrap()
            .timestamp();
        let thursday_ms = chrono::DateTime::parse_from_rfc3339("2026-09-17T12:34:56Z")
            .unwrap()
            .timestamp_millis();
        assert_eq!(
            trade_bar_open_time(thursday_ms, Resolution::Week),
            Some(monday)
        );
        let month_ms = chrono::DateTime::parse_from_rfc3339("2026-09-17T12:34:56Z")
            .unwrap()
            .timestamp_millis();
        let month_open = chrono::DateTime::parse_from_rfc3339("2026-09-01T00:00:00Z")
            .unwrap()
            .timestamp();
        assert_eq!(
            trade_bar_open_time(month_ms, Resolution::Month),
            Some(month_open)
        );
    }

    #[test]
    fn indicator_trade_bar_time_matches_crypto_chart_close_boundaries() {
        assert_eq!(
            trade_bar_time(1_789_375_265_432, Resolution::Minute1),
            Some(1_789_375_320)
        );
        let thursday_ms = chrono::DateTime::parse_from_rfc3339("2026-09-17T12:34:56Z")
            .unwrap()
            .timestamp_millis();
        let next_monday = chrono::DateTime::parse_from_rfc3339("2026-09-21T00:00:00Z")
            .unwrap()
            .timestamp();
        assert_eq!(trade_bar_time(thursday_ms, Resolution::Week), Some(next_monday));
        let next_month = chrono::DateTime::parse_from_rfc3339("2026-10-01T00:00:00Z")
            .unwrap()
            .timestamp();
        assert_eq!(trade_bar_time(thursday_ms, Resolution::Month), Some(next_month));
    }
}
