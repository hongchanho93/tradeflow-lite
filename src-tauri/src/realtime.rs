...
#[cfg(test)]
mod tests {
    use super::{RealtimeState, RealtimeTradePayload};
    ...
    #[test]
    fn aggregate_trade_source_ids_are_serialized_for_indicator_consumers() {
        let payload = RealtimeTradePayload {
            request_id: 1,
            provider_id: "binance_spot",
            symbol: "BINANCE:BTCUSDT".to_string(),
            resolution: crate::contracts::Resolution::Minute1,
            sequence: Some(7),
            trade_id: 5,
            first_trade_id: Some(11),
            last_trade_id: Some(13),
            trade_time_ms: 1_000,
            bar_time: 60,
            price: 100.0,
            quantity: 2.0,
            side: Some(std::borrow::Cow::Borrowed("buy")),
            flags: None,
        };
        let value = serde_json::to_value(payload).expect("serialize realtime trade");
        assert_eq!(value["firstTradeId"], 11);
        assert_eq!(value["lastTradeId"], 13);
    }
}
