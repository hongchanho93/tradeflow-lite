...
use crate::contracts::{Bar, MarketSeriesKind, ProbabilityPoint, Symbol};
#[cfg(tradeflow_tdx)]
use crate::contracts::{Adjustment, AppError, Resolution, SymbolKind};
#[cfg(tradeflow_tdx)]
use tradeflow_tdx::standard::{Market, SecurityQuote, SecurityQuotes, Transactions};
#[cfg(tradeflow_tdx)]
...
#[cfg(tradeflow_tdx)]
pub use transactions::RecentTrade;

#[cfg(tradeflow_tdx)]
const MAX_HISTORY_BARS: usize = 12_000;
#[cfg(tradeflow_tdx)]
const SOURCE: &str = "tradeflow-tdx";
static ACTIVE_HISTORY_INCARNATION: AtomicU64 = AtomicU64::new(0);

thread_local! {
    static HISTORY_REQUEST_INCARNATION: Cell<u64> = const { Cell::new(0) };
}

pub(crate) fn activate_history_incarnation(incarnation: u64) -> bool {
    if incarnation == 0 {
        return false;
    }
    let mut current = ACTIVE_HISTORY_INCARNATION.load(Ordering::Acquire);
    loop {
        ...
        match ACTIVE_HISTORY_INCARNATION.compare_exchange_weak(
            current,
            incarnation,
            Ordering::AcqRel,
            Ordering::Acquire,
...
pub(crate) fn with_history_incarnation<T>(incarnation: u64, run: impl FnOnce() -> T) -> T {
    HISTORY_REQUEST_INCARNATION.with(|slot| {
        let previous = slot.replace(incarnation);
        struct Reset<'a> {
            slot: &'a Cell<u64>,
            previous: u64,
        }
...
pub(crate) fn history_request_is_active() -> bool {
    if crate::market_query::is_independent() { return crate::market_query::check_active().is_ok(); }
    HISTORY_REQUEST_INCARNATION.with(|slot| {
        let incarnation = slot.get();
        incarnation == 0 || ACTIVE_HISTORY_INCARNATION.load(Ordering::Acquire) == incarnation
    })
}
...
pub(crate) fn fetch_history_bars_raw(
    ...
    let success = hosts::run_with_failover(&hosts, |host| {
        if !history_request_is_active() {
            return Err("history_request_cancelled".to_string());
        }
        hosts::connect_and_run(host, |client| history::load_history(client, &query, today))
    })
    .map_err(|attempts| {
        if !history_request_is_active() {
            return AppError::new("history_request_cancelled", "历史行情请求已被新选择替代");
        }
...
pub fn fetch_recent_trades(
    ...
    if !matches!(kind, SymbolKind::Stock | SymbolKind::Etf) {
        return Err(AppError::new(
            ...
            "TDX 逐笔成交当前只用于股票和 ETF",
        ));
    }
    let count = u16::try_from(count.clamp(1, 800)).expect("clamped transaction count");
    let (exchange, code) = symbol.parts();
    let market = tdx_market(exchange)?;
    let security_code = SecurityCode::new(code)
        .map_err(|error| AppError::new("invalid_symbol", error.to_string()))?;
    ...
    .map_err(|attempts| {
        ...
        AppError::new(
            "market_data_unavailable",
            format!("all Lite hosts failed for recent trades; {detail}"),
        )
    })?;
    hosts::record_attempts(Some(&success.host), &success.attempts);
    ...
    Ok(RecentTradesResponse {
        ...
        received_at,
        trades: transactions::normalize(success.value, &kind, clock::shanghai_today()),
        diagnostics: HistoryDiagnostics {
            source: SOURCE,
            host: success.host,
            latency_ms: success.latency_ms,
...
mod tests {
    ...
    use tradeflow_tdx::standard::{BookLevel, Market, SecurityQuote};
