...
use tradeflow_tdx::standard::{Market, SecurityQuotes};
...
fn assert_quote_contract(
    ...
    if matches!(kind, SymbolKind::Stock | SymbolKind::Etf) {
        assert!(
            !quote.bids.is_empty() && !quote.asks.is_empty(),
            "{}: active stock/ETF quote must carry TDX order-book levels",
            symbol.as_str()
        );
    }
}
    ...
    for (exchange, code, kind) in tdx_representative_cases() {
        ...
        match router.fetch_quote(QuoteRequest {
            ...
            Ok(response) => {
                ...
                println!(
                    "adapter.real.quote provider=tdx symbol={} kind={} state=valid last={:.6} previous_close={:.6} bids={} asks={} best_bid={:?} best_ask={:?} source={} host={}",
                    symbol.as_str(),
                    kind.as_str(),
                    response.quote.last,
                    response.quote.previous_close,
                    response.quote.bids.len(),
                    response.quote.asks.len(),
                    response.quote.bids.first().map(|level| level.price),
                    response.quote.asks.first().map(|level| level.price),
                    response.source,
...
#[cfg(feature = "provider-binance")]
#[derive(Default)]
struct SoakRealtimeSink {
    events: Mutex<Vec<RealtimeEventEnvelope>>,
}

#[cfg(feature = "provider-binance")]
impl SoakRealtimeSink {
    fn snapshot(&self) -> Vec<RealtimeEventEnvelope> {
        self.events
            .lock()
            .expect("soak realtime sink mutex poisoned")
            .clone()
    }
}

#[cfg(feature = "provider-binance")]
impl RealtimeSink for SoakRealtimeSink {
    fn emit(&self, event: RealtimeEventEnvelope) -> Result<(), AppError> {
        self.events
            .lock()
            .expect("soak realtime sink mutex poisoned")
...
#[cfg(feature = "provider-binance")]
#[test]
#[ignore = "connects to Binance public WebSocket streams and rapidly switches BTC -> ETH -> BTC"]
fn real_adapter_binance_fast_switch_a_b_a() {
    let router = MarketRouter::builtin().expect("built-in adapter registry must be valid");
    let active_request_id = Arc::new(AtomicU64::new(9_300));
    let sink = Arc::new(RecoverySink::with_active(Arc::clone(&active_request_id)));
    let btc = Symbol::new("BINANCE", "BTCUSDT").unwrap();
    let eth = Symbol::new("BINANCE", "ETHUSDT").unwrap();
    ...
    start(9_300, btc.clone());
    let first_deadline = Instant::now() + REALTIME_TIMEOUT;
    while !sink.accepted().iter().any(|event| {
        event.request_id == 9_300 && matches!(event.payload, RealtimePayload::Bar { .. })
    }) {
        assert!(
            Instant::now() < first_deadline,
            "initial BTC stream did not produce a bar"
        );
        thread::sleep(Duration::from_millis(20));
    }

    active_request_id.store(9_301, Ordering::Release);
    start(9_301, eth);
    active_request_id.store(9_302, Ordering::Release);
    start(9_302, btc.clone());

    let final_deadline = Instant::now() + REALTIME_TIMEOUT;
    while !sink.accepted().iter().any(|event| {
        event.request_id == 9_302 && matches!(event.payload, RealtimePayload::Bar { .. })
    }) {
        assert!(
            Instant::now() < final_deadline,
            "final BTC stream did not recover after A -> B -> A"
        );
        thread::sleep(Duration::from_millis(20));
    }
    let middle_events_before_quiet = sink
        .accepted()
        .iter()
        .filter(|event| event.request_id == 9_301)
        .count();
    thread::sleep(Duration::from_millis(500));
    let accepted = sink.accepted();
    let middle_events_after_quiet = accepted
        .iter()
        .filter(|event| event.request_id == 9_301)
        .count();
    assert_eq!(
        middle_events_after_quiet, middle_events_before_quiet,
        "the superseded B request must stop contributing accepted events after final A is active"
    );
    assert!(accepted.iter().any(|event| {
        event.request_id == 9_302
            && event.symbol == btc
            && matches!(event.payload, RealtimePayload::Bar { .. })
    }));
    assert!(
        accepted
            .iter()
            .filter(|event| event.request_id == 9_302)
            .all(|event| event.symbol == btc && event.provider_id == "binance_spot")
    );
    active_request_id.store(9_303, Ordering::Release);
    println!(
        "adapter.real.fast_switch provider=binance_spot sequence=BTC->ETH->BTC accepted={} middle_after_final={} stale_rejected={}",
        accepted.len(),
        middle_events_after_quiet,
...
#[cfg(feature = "provider-binance")]
#[test]
#[ignore = "runs a Binance public WebSocket soak through MarketRouter (60s by default)"]
fn real_adapter_binance_realtime_soak_60s() {
    let router = MarketRouter::builtin().expect("built-in adapter registry must be valid");
    let request_id = 9_400;
    let active_request_id = Arc::new(AtomicU64::new(request_id));
    let sink = Arc::new(SoakRealtimeSink::default());
    ...
    let soak_seconds = std::env::var("TRADEFLOW_TEST_SOAK_SECONDS")
        .ok()
    ...
    for event in &events {
        ...
        if let Some(previous) = last_sequence_by_channel.insert(channel.clone(), sequence) {
            assert!(
                sequence > previous,
                "{channel} sequence regressed during realtime soak: {sequence} <= {previous}"
            );
        }
    }
    ...
    assert_eq!(
        ...
        1,
        "Binance soak worker must release the sink after cancellation"
    );
    let count_after_cancel = sink.snapshot().len();
    thread::sleep(Duration::from_millis(250));
    assert_eq!(sink.snapshot().len(), count_after_cancel);
    println!(
        "adapter.real.soak provider=binance_spot symbol={} duration_s={} events={} channels={} sink_released=true",
        symbol.as_str(),
...
fn deterministic_adapter_recovery_rejects_stale_and_sequence_regressions() {
    ...
    while !sink.accepted().iter().any(|event| {
        ...
        assert!(
            ...
            "fake reconnect did not reach connected state"
        );
