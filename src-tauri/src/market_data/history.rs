...
use tradeflow_tdx::standard::{
    BarPeriod, IndexBars, MAX_BARS_PER_REQUEST, Market, RawBar, SecurityBars, SecurityQuotes,
    Standard, XdxrDetail, XdxrEntry, XdxrInfo,
};
use tradeflow_tdx::{SecurityCode, Session};

/// 按周/月聚合前复权数据时，最多向主站请求的日线条数。
const MAX_PERIOD_AGGREGATION_SOURCE_BARS: usize = 8_000;
/// TDX 分页 offset 是 u16；120/240m 深历史的 60m 源数据必须按目标条数单独扩容，
/// 不能复用目标聚合 K 线的 8k 上限。
const MAX_INTRADAY_AGGREGATION_SOURCE_BARS: usize = u16::MAX as usize;
...
fn intraday_source_count(resolution: Resolution, target_count: usize) -> usize {
    let source_bars_per_bar = match resolution {
        Resolution::Minute120 => 2,
        Resolution::Minute240 => 4,
        _ => return target_count,
    };
    // 多取一段余量，避免最近交易日只有部分 60 分钟线时，结果不足请求条数。
    target_count
        .saturating_mul(source_bars_per_bar)
        .saturating_add(16)
        .min(MAX_INTRADAY_AGGREGATION_SOURCE_BARS)
}
...
pub(crate) fn load_history<S: Session<Standard>>(
    ...
    let native_count = if aggregate_period {
        ...
        MAX_PERIOD_AGGREGATION_SOURCE_BARS.min(query.count * days_per_bar + 60)
    } else if aggregate_intraday {
        intraday_source_count(query.resolution, query.count)
    } else {
        query.count
    };
...
fn fetch_native_bars<S: Session<Standard>>(
    ...
    period: BarPeriod,
    count: usize,
) -> Result<Vec<RawBar>, tradeflow_tdx::TdxError> {
    let mut rows = Vec::with_capacity(count);
    while rows.len() < count {
        if !super::history_request_is_active() {
            return Err(tradeflow_tdx::TdxError::InvalidArgument(
                "history_request_cancelled".to_string(),
            ));
        }
        let page_size = usize::from(MAX_BARS_PER_REQUEST).min(count - rows.len());
        let offset = u16::try_from(rows.len()).map_err(|_| {
            tradeflow_tdx::TdxError::InvalidArgument(format!("bar offset {} is too large", rows.len()))
        })?;
...
fn normalize(raw: &[RawBar], resolution: Resolution) -> Result<Vec<Bar>, String> {
    ...
    let mut bars = raw
        ...
        .map(|row| {
            ...
            .ok_or_else(|| format!("invalid bar date {:?}", row.time))?;
            let time = if matches!(resolution, Resolution::Week | Resolution::Month) {
                aggregate_period_identity_time(day, resolution)
                    .ok_or_else(|| format!("invalid aggregate bar time {:?}", row.time))?
            } else {
...
/// 周/月 K 线使用周期本身的固定时间作为 identity，而不是“当前最后一个交易日”。
/// 这样当前周/月在新增日线时不会不断换 key，前复权聚合与 TDX 原生周/月也保持一致。
fn aggregate_period_identity_time(date: NaiveDate, resolution: Resolution) -> Option<i64> {
    let identity_date = match resolution {
        Resolution::Week => date.checked_sub_signed(Duration::days(i64::from(
            date.weekday().num_days_from_monday(),
        )))?,
        Resolution::Month => NaiveDate::from_ymd_opt(date.year(), date.month(), 1)?,
        _ => return None,
    };
    shanghai_timestamp(identity_date, 15, 0)
}
...
fn aggregate_bars(bars: Vec<Bar>, resolution: Resolution) -> Vec<Bar> {
    ...
    for mut bar in bars {
        ...
        let Some(&index) = index_by_key.get(&key) else {
            bar.time = aggregate_period_identity_time(date, resolution)
                .expect("valid aggregate identity for an existing bar");
...
fn aggregate_intraday_bars(bars: Vec<Bar>, resolution: Resolution) -> Vec<Bar> {
    ...
    for mut bar in bars {
        ...
        let Some(&index) = index_by_key.get(&key) else {
            // 保留 TDX 的“周期收盘时间”语义，但从第一根子 K 开始就使用最终
            // bucket 的固定时间，避免形成中的同一根 K 线不断改变 identity。
            bar.time = match (width, bucket) {
                (2, 0) => shanghai_timestamp(date, 11, 30),
                (2, 1) | (4, 0) => shanghai_timestamp(date, 15, 0),
                _ => None,
            }
            .expect("valid intraday aggregate identity for an existing bar");
            index_by_key.insert(key, groups.len());
            groups.push(bar);
            continue;
        };
...
mod tests {
    ...
    use super::{
        ...
        intraday_source_count, load_history, normalize, qfq_events, repair_index_ohlc_envelopes,
    ...
    use tradeflow_tdx::{Request, SecurityCode, Session, TdxDate, TdxDateTime, TdxError};
    ...
    fn index_and_security_use_distinct_commands() {
        ...
        session
            .responses
            .push_back(Box::new(Vec::<tradeflow_tdx::standard::IndexBar>::new()));
    ...
    fn qfq_cash_dividend_and_weekly_aggregation() {
        ...
        assert_eq!(
            ...
            (ts(2026, 9, 7, 15), Some(3.0))
        );
    }

    #[test]
    fn aggregate_period_identity_does_not_move_while_period_is_forming() {
        let monday = Bar::new(
            ts(2026, 9, 14, 15),
            10.0,
            11.0,
        ...
        let weekly_first = aggregate_bars(vec![monday.clone()], Resolution::Week);
        let weekly_later = aggregate_bars(vec![monday.clone(), tuesday.clone()], Resolution::Week);
        assert_eq!(weekly_first[0].time, weekly_later[0].time);
        assert_eq!(weekly_later[0].time, ts(2026, 9, 14, 15));

        let monthly_first = aggregate_bars(vec![monday], Resolution::Month);
        let monthly_later = aggregate_bars(vec![tuesday], Resolution::Month);
        assert_eq!(monthly_first[0].time, monthly_later[0].time);
        assert_eq!(monthly_later[0].time, ts(2026, 9, 1, 15));
    }
    ...
    #[test]
    fn deep_intraday_history_sizes_source_bars_from_target_bars() {
        assert_eq!(
            intraday_source_count(Resolution::Minute120, 8_000),
            16_016,
            "8k two-hour target bars need more than the old 8k 60-minute source cap",
        );
        assert_eq!(
            intraday_source_count(Resolution::Minute240, 8_000),
            32_016,
            "8k four-hour target bars need four 60-minute source bars per target plus margin",
        );
        assert_eq!(
            intraday_source_count(Resolution::Minute60, 8_000),
            8_000,
            "native intervals must not be multiplied by aggregate-source sizing",
        );
    }
    ...
    #[test]
    fn intraday_aggregate_identity_does_not_move_while_bar_is_forming() {
        let first = Bar::new(
            ts(2026, 9, 14, 10) + 30 * 60,
            1.0,
            2.0,
        ...
        let two_hour_first =
            aggregate_intraday_bars(vec![first.clone()], Resolution::Minute120);
        let two_hour_later =
            aggregate_intraday_bars(vec![first.clone(), second.clone()], Resolution::Minute120);
        assert_eq!(two_hour_first[0].time, two_hour_later[0].time);
        assert_eq!(two_hour_later[0].time, ts(2026, 9, 14, 11) + 30 * 60);

        let four_hour_first = aggregate_intraday_bars(vec![first], Resolution::Minute240);
        let four_hour_later =
            aggregate_intraday_bars(vec![second, afternoon], Resolution::Minute240);
        assert_eq!(four_hour_first[0].time, four_hour_later[0].time);
        assert_eq!(four_hour_later[0].time, ts(2026, 9, 14, 15));
    }
