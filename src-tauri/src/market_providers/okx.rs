...
fn fetch_history(market: OkxMarket, request: HistoryRequest) -> Result<HistoryResponse, AppError> {
    ...
    while bars.len() < requested {
        crate::market_query::check_active().map_err(|code| AppError::new(code, "独立行情请求已停止"))?;
...
#[cfg(test)]
static TEST_FORCE_SOCKET_DROP_REQUEST_ID: AtomicU64 = AtomicU64::new(0);
#[cfg(test)]
static TEST_FORCE_SOCKET_DROP_MASK: AtomicU8 = AtomicU8::new(0);

#[cfg(test)]
fn take_test_force_socket_drop(kind: WsKind, request_id: u64) -> bool {
    if TEST_FORCE_SOCKET_DROP_REQUEST_ID.load(Ordering::Acquire) != request_id {
        return false;
    }
    let bit = match kind {
        WsKind::Public => 0b01,
        WsKind::Business => 0b10,
    };
    TEST_FORCE_SOCKET_DROP_MASK.fetch_and(!bit, Ordering::AcqRel) & bit != 0
}
    ...
    loop {
        ...
        match read_result {
            Ok(Message::Text(text)) => {
                ...
                #[cfg(test)]
                if take_test_force_socket_drop(kind, active.request.request_id) {
                    emit_reconnecting(
                        &active.request,
                        &active.sink,
                        format!("test-forced OKX {kind:?} WebSocket disconnect"),
                    );
                    socket = None;
                }
            }
                ...
                sink.emit(
                    request.envelope(
                        ...
                        RealtimePayload::Trade {
                            ...
                            last_trade_id: None,
                            trade_time_ms,
                            bar_time: trade_bar_time(trade_time_ms, request.resolution)
                                .unwrap_or_default(),
                            price,
                            quantity,
...
mod tests {
    ...
    use super::{
        ContractSpec, OkxMarket, OkxRealtimeBarState, SequenceClock, TEST_FORCE_SOCKET_DROP_MASK,
        TEST_FORCE_SOCKET_DROP_REQUEST_ID, WsKind, candle_close_time, emit_ws_payload,
        interval_for, is_control_payload, market_quantity, parse_candle,
    };
    ...
    impl ProbeSink {
        ...
        fn wait_until(
            &self,
            timeout: Duration,
            predicate: impl Fn(&[RealtimeEventEnvelope]) -> bool,
        ) -> bool {
            let deadline = Instant::now() + timeout;
            let mut events = self.events.lock().unwrap();
    ...
    #[test]
    #[ignore = "runs an OKX public WebSocket soak through MarketRouter (60s by default)"]
    fn okx_real_realtime_soak() {
        let router = MarketRouter::builtin().unwrap();
        let request_id = 21_050;
        let active = Arc::new(AtomicU64::new(request_id));
        let sink = Arc::new(ProbeSink::default());
        let symbol = Symbol::new("OKX", "BTC-USDT").unwrap();
        router
            .start_realtime(
                RealtimeRequest {
                    request_id,
        ...
        let soak_seconds = std::env::var("TRADEFLOW_TEST_OKX_SOAK_SECONDS")
            .ok()
        ...
        assert!(
            events
                ...
                .filter(|event| matches!(
                    event.payload,
                    RealtimePayload::Status { status: "connected", .. }
                ))
                .count()
                >= 2,
            "OKX soak did not connect both Public and Business sockets: {events:?}"
        );
        assert!(
            events
                .iter()
        ...
        assert!(
            events
                .iter()
                .any(|event| matches!(event.payload, RealtimePayload::Trade { .. }))
        );
        assert!(
            events
                .iter()
                .any(|event| matches!(event.payload, RealtimePayload::Bar { .. }))
        );
        assert!(events.iter().all(|event| {
            event.request_id == request_id
                && event.provider_id == "okx_spot"
                && event.symbol == symbol
        }));

        let mut last_sequence_by_channel = HashMap::<String, u64>::new();
        for event in &events {
            let Some(sequence) = event.sequence else {
                continue;
            };
            let channel = match &event.payload {
                RealtimePayload::Bar { source, .. } => format!("bar:{source}"),
                RealtimePayload::Point { source, .. } => format!("point:{source}"),
                RealtimePayload::Trade { .. } => "trade".to_string(),
                RealtimePayload::Depth { .. } => "depth".to_string(),
                RealtimePayload::Status { .. } => continue,
            };
            if let Some(previous) = last_sequence_by_channel.insert(channel.clone(), sequence) {
                assert!(
                    sequence > previous,
                    "{channel} sequence regressed during OKX realtime soak: {sequence} <= {previous}"
                );
            }
        }

        active.store(request_id + 1, Ordering::Release);
        let release_deadline = Instant::now() + Duration::from_secs(4);
        ...
        assert_eq!(
            Arc::strong_count(&sink),
            1,
            "OKX soak workers did not release after cancellation"
        );
        let count_after_cancel = sink.snapshot().len();
        std::thread::sleep(Duration::from_millis(250));
        assert_eq!(sink.snapshot().len(), count_after_cancel);
        println!(
            "okx.real.soak provider=okx_spot symbol={} duration_s={} events={} channels={} sink_released=true",
            symbol.as_str(),
            started.elapsed().as_secs(),
            events.len(),
            last_sequence_by_channel.len(),
        );
    }

    #[test]
    #[ignore = "rapidly switches a real OKX public WebSocket subscription BTC -> ETH -> BTC"]
    fn okx_real_fast_switch_a_b_a() {
        let router = MarketRouter::builtin().unwrap();
        let active = Arc::new(AtomicU64::new(21_080));
        let sink = Arc::new(ProbeSink::default());
        let btc = Symbol::new("OKX", "BTC-USDT").unwrap();
        let eth = Symbol::new("OKX", "ETH-USDT").unwrap();

        let start = |request_id: u64, symbol: Symbol| {
            router
                .start_realtime(
                    RealtimeRequest {
                        request_id,
        ...
        let wait_for_both_streams = |request_id: u64, symbol: &Symbol| {
            sink.wait_until(Duration::from_secs(15), |events| {
                let current = events
                    .iter()
                    .filter(|event| event.request_id == request_id && event.symbol == *symbol)
                    .collect::<Vec<_>>();
                current.iter().any(|event| {
                    matches!(
                        event.payload,
                        RealtimePayload::Bar { .. } | RealtimePayload::Trade { .. }
                    )
                }) && current
                    .iter()
                    .any(|event| matches!(event.payload, RealtimePayload::Depth { .. }))
        ...
        start(21_080, btc.clone());
        assert!(
            wait_for_both_streams(21_080, &btc),
            "initial BTC subscription did not receive both OKX streams: {:?}",
            sink.snapshot()
        );

        active.store(21_081, Ordering::Release);
        start(21_081, eth.clone());
        assert!(
            wait_for_both_streams(21_081, &eth),
            "ETH replacement did not receive both OKX streams: {:?}",
            sink.snapshot()
        );

        active.store(21_082, Ordering::Release);
        start(21_082, btc.clone());
        assert!(
            wait_for_both_streams(21_082, &btc),
            "final BTC subscription did not recover both OKX streams: {:?}",
            sink.snapshot()
        );
        let final_ready_len = sink.snapshot().len();
        std::thread::sleep(Duration::from_millis(500));
        let events = sink.snapshot();
        assert!(
            events[final_ready_len..]
                .iter()
                .all(|event| event.request_id != 21_081 && event.symbol != eth),
            "superseded ETH events continued after final BTC was fully active: {:?}",
            &events[final_ready_len..]
        );
        assert!(events.iter().any(|event| {
            event.request_id == 21_082
                && event.symbol == btc
                && matches!(event.payload, RealtimePayload::Bar { .. } | RealtimePayload::Trade { .. })
        }));
        assert!(events.iter().any(|event| {
            event.request_id == 21_082
                && event.symbol == btc
                && matches!(event.payload, RealtimePayload::Depth { .. })
        }));

        active.store(21_083, Ordering::Release);
        let release_deadline = Instant::now() + Duration::from_secs(4);
        while Arc::strong_count(&sink) > 1 && Instant::now() < release_deadline {
            std::thread::sleep(Duration::from_millis(50));
        }
        assert_eq!(
            Arc::strong_count(&sink),
            1,
            "OKX fast-switch workers did not release after cancellation"
        );
        println!(
            "okx.real.fast_switch sequence=BTC->ETH->BTC events={} sink_released=true",
            events.len()
        );
    }

    #[test]
    #[ignore = "forces both real OKX public WebSocket connections to drop and verifies provider recovery"]
    fn okx_real_forced_disconnect_reemits_connected() {
        let router = MarketRouter::builtin().unwrap();
        let request_id = 21_100;
        let active = Arc::new(AtomicU64::new(request_id));
        let sink = Arc::new(ProbeSink::default());
        router
            .start_realtime(
        ...
        assert!(
            ...
            "both OKX sockets did not reach their initial connected state: {:?}",
            sink.snapshot()
        );
        let initial_connected = sink
            .snapshot()
            .iter()
            .filter(|event| {
                matches!(
            ...
            .count();
        TEST_FORCE_SOCKET_DROP_MASK.store(0b11, Ordering::Release);
        TEST_FORCE_SOCKET_DROP_REQUEST_ID.store(request_id, Ordering::Release);

        let recovered = sink.wait_until(Duration::from_secs(15), |events| {
            let forced_reconnecting = events
                .iter()
                .enumerate()
                .filter_map(|(index, event)| {
                    matches!(
                        &event.payload,
                        RealtimePayload::Status { status: "reconnecting", message: Some(message) }
                            if message.contains("test-forced")
                    )
                    .then_some(index)
                })
                .collect::<Vec<_>>();
            if forced_reconnecting.len() < 2 {
                return false;
            }
            let connected_count = events
                ...
                .count();
            if connected_count < initial_connected + 2 {
                return false;
            }
            let last_forced = *forced_reconnecting.last().unwrap();
            let tail = &events[last_forced + 1..];
            let market_stream_recovered = tail.iter().any(|event| {
                matches!(
                    event.payload,
                    RealtimePayload::Bar { .. } | RealtimePayload::Trade { .. }
                )
            });
            let depth_stream_recovered = tail
                .iter()
                .any(|event| matches!(event.payload, RealtimePayload::Depth { .. }));
            market_stream_recovered && depth_stream_recovered
        });
        let events = sink.snapshot();
        active.store(request_id + 1, Ordering::Release);
        TEST_FORCE_SOCKET_DROP_REQUEST_ID.store(0, Ordering::Release);
        TEST_FORCE_SOCKET_DROP_MASK.store(0, Ordering::Release);
        assert!(
            recovered,
            "OKX did not restore both market and depth streams after forced dual disconnect: {events:?}"
        );
    }
}
