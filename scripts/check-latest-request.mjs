...
const abaGate = new LatestRequestGate();
const firstA = abaGate.begin();
const b = abaGate.begin();
const secondA = abaGate.begin();
assert.equal(firstA, 1);
assert.equal(b, 2);
assert.equal(secondA, 3);
assert.equal(
  abaGate.isCurrent(firstA),
  false,
  'A -> B -> A must keep the first A incarnation stale even when the visible selection matches A again',
);
assert.equal(abaGate.isCurrent(secondA), true);
