...
assert.match(adapter, /pub const ADAPTER_CONTRACT_VERSION: &str = "3"/);
assert.match(adapter, /fn replace_subscription\(/);
...
assert.match(
  lib,
  /async fn benchmark_hosts\(router: State<'_, MarketRouter>\)[\s\S]*router\.require_provider_enabled\("tdx"\)\?[\s\S]*spawn_blocking\(market_data::benchmark_hosts\)/,
  'host benchmarking must require the enabled TDX policy and run as an async Tauri command backed by a blocking worker',
);
...
assert.match(okx, /const HISTORY_PAGE_SIZE: usize = 300/);
assert.match(okx, /struct OkxRealtimeManager/);
assert.match(okx, /connection_reused=/);
assert.match(okx, /"unsubscribe"/);
