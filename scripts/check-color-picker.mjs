...
const pickerStyles = await import('node:fs/promises').then(({ readFile }) => readFile(new URL('../src/color-picker.css', import.meta.url), 'utf8'));
assert.match(
  pickerStyles,
  /\.indicator-input-field \.tf-color-trigger\s*\{[^}]*width:\s*34px;[^}]*height:\s*34px;/,
  'indicator color triggers must be square',
);
assert.match(
  await import('node:fs/promises').then(({ readFile }) => readFile(new URL('../src/main.ts', import.meta.url), 'utf8')),
  /data-chart-appearance-color="backgroundColor"[^>]*data-tf-color-opacity-target="chart-settings-background-opacity"/,
  'layout colors must reuse the TF palette with opacity control',
);
