...
function streamedBody(chunks) {
  ...
  const body = new ReadableStream({
    pull(controller) {
      if (state.reads === chunks) { controller.close(); return; }
      state.reads++;
...
for (const [label, chunks, declared, expectedStatus, expectedReads, cancelled] of [
  ['unknown length overflow', 6, null, 502, 5, true],
  ['declared overflow', 6, 6 * 1024 * 1024, 502, 0, true],
  ['exact boundary', 4, null, 200, 4, false],
  ['small response', 1, 1024 * 1024, 200, 1, false],
]) test(`Cloudflare bounded stream: ${label}`, async () => {
  const { state, body } = streamedBody(chunks);
  const headers = { 'content-type': 'application/json' };
  if (declared !== null) headers['content-length'] = String(declared);
  const response = await handleRequest(new Request('https://example.test/v1/polymarket/clob/midpoint?token_id=123'), {}, ctx, {
    cache: null, fetchImpl: async () => new Response(body, { headers }),
  });
  assert.equal(response.status, expectedStatus);
  if (expectedStatus === 502) assert.equal((await response.json()).error, 'upstream_response_too_large');
  else assert.equal((await response.arrayBuffer()).byteLength, chunks * 1024 * 1024);
  assert.equal(state.reads, expectedReads, 'stop reading at the first oversized chunk');
  assert.equal(state.cancelled, cancelled, 'release an oversized upstream immediately');
});
