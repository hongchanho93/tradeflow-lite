...
export class DrawingHistory {
  ...
  /** Host-only rollback checkpoint; does not pop the user's Undo/Redo history. */
  checkpoint(): { readonly past: readonly string[]; readonly future: readonly string[] } {
    return Object.freeze({ past: Object.freeze([...this.past]), future: Object.freeze([...this.future]) });
  }

  restore(checkpoint: { readonly past: readonly string[]; readonly future: readonly string[] }): void {
    if (!checkpoint.past.length || checkpoint.past.length > this.limit + 1 || checkpoint.future.length > this.limit
        || [...checkpoint.past, ...checkpoint.future].some(value => typeof value !== 'string')) throw new Error('invalid drawing history checkpoint');
    this.past = [...checkpoint.past]; this.future = [...checkpoint.future];
  }
}
