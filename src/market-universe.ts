...
export function mergeMarketCatalogPage(
  rows: readonly MarketSymbol[],
  page: readonly MarketSymbol[],
  providerId: string,
  venue: string,
  firstPage: boolean,
  bundledKeys: ReadonlySet<string>,
): MarketSymbol[] {
  const incoming = new Set(page.map((item) => marketProviderKey(item.providerId, item.symbol)));
  return [
    ...rows.filter((item) => {
      const key = marketProviderKey(item.providerId, item.symbol);
      if (incoming.has(key)) return false;
      if (firstPage && item.providerId === providerId && item.venue === venue) {
        return bundledKeys.has(key);
      }
      return true;
    }),
    ...page,
