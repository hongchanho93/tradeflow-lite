...
export function retainRealtimeHistory<T>(bars: T[], resolution: string) {
  const retainBars = deepHistoryBars(resolution);
  const maxBars = retainBars + Math.ceil(retainBars / 4);
  if (bars.length <= maxBars) return { bars, trimmed: 0, retainBars, maxBars };
  const retained = bars.slice(-retainBars);
  return {
    bars: retained,
    trimmed: bars.length - retained.length,
    retainBars,
    maxBars,
  };
}
